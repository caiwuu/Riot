...
- **权限与沙箱**：默认走「全部放行」——常规操作不打断你，危险操作（SSH 密钥、shell 启动脚本、hooks 这类写下去就等于交出执行权的目标）仍然拦下来问。会话可在「每次询问 / 自动接受编辑 / 规划 / 自动判危 / 全部放行 / 无人值守」之间切换；命令可限制在工作区里改文件，也可再掐掉联网 —— macOS 用系统自带的隔离，开箱就开着，Windows 那套要先装一次，默认不开。
