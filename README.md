...
src-tauri/              Tauri 宿主：窗口、内核进程监管、PTY、文件系统
...
M2 阶段，**能对着真实模型跑起来了**。548 个测试 + 11 个黄金用例 + 53 个变异 + 500 seed 混沌长跑。已完成：
...
还没接的：分层压缩管道（目前只有"清理旧工具结果"这一档）、Glob / WebFetch 之类的次要工具、SQLite 持久化、多会话。
