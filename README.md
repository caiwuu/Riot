...
| **Claude Code** | Skills、斜杠命令、Hooks、MCP、项目记忆；分档的权限模型，规划模式先写计划再动手            |
...
- **权限与沙箱**：默认走「全部放行」——常规操作不打断你，危险操作（SSH 密钥、shell 启动脚本、hooks 这类写下去就等于交出执行权的目标）仍然拦下来问。会话可在「每次询问 / 编辑放行 / 规划 / 自动判危 / 全部放行 / 无人值守」之间切换；命令可限制在工作区里改文件，也可再掐掉联网 —— macOS 用系统自带的隔离，开箱就开着，Windows 那套要先装一次，默认不开。
- **规划后再动手**：规划模式只读侦察，把计划写成项目里的 `.riot/plans/*.plan.md`，在右侧面板里审阅；在输入框里提意见让它改，点「构建」才开始改代码。任务大、含糊、有取舍时，agent 模式下模型会先建议切到规划，你点「切换」或不切。
