import { DEFAULT_RENDERER_DEFINITIONS, DEFAULT_SUPPORTED_EXTENSIONS } from '../registry/formats.js';
import { getExtension, normalizeFileExtension, normalizeFilename, normalizeSource, } from './index.js';
...
const startsWithBytes = (bytes, expected) => (expected.every((value, index) => bytes[index] === value));
...
const collectCentralDirectoryEntries = (bytes) => {
...
const inspectZipBlob = async (blob, maxPackageMetadataBytes) => {
    if (blob.size < 22) {
        return { signatureValid: false, complete: true, entries: new Set() };
    }
...
const inspectZip = async (source, maxPackageMetadataBytes) => {
    if (source.buffer) {
...
const readPrefix = async (source, byteLength = 1024) => {
    if (source.buffer) {
...
const inspectStructuredContent = async (source, extension, maxPackageMetadataBytes) => {
    var _a, _b, _c;
    const size = (_b = (_a = source.buffer) === null || _a === void 0 ? void 0 : _a.byteLength) !== null && _b !== void 0 ? _b : (_c = source.file) === null || _c === void 0 ? void 0 : _c.size;
 ...
 */
export const precheckFileViewerSource = async (input, options = {}) => {
    var _a;
    const source = toSource(input, options);
    const normalized = normalizeSource(source);
    const extension = normalizeFileExtension(options.type || normalized.extension || getExtension(options.filename || normalized.filename));
    const filename = normalizeFilename(options.filename || normalized.filename);
    const supportedExtensions = new Set(Array.from(options.supportedExtensions || DEFAULT_SUPPORTED_EXTENSIONS, normalizeFileExtension));
    const supported = supportedExtensions.has(extension);
    const rendererId = ((_a = DEFAULT_RENDERER_DEFINITIONS.find(definition => definition.extensions.some(candidate => normalizeFileExtension(candidate) === extension))) === null || _a === void 0 ? void 0 : _a.id) || null;
    if (!supported) {
        return {
            filename,
            extension,
    ...
    const inspection = await inspectStructuredContent(source, extension, Math.max(64 * 1024, options.maxPackageMetadataBytes || DEFAULT_MAX_PACKAGE_METADATA_BYTES));
