...
export const normalizeFileExtension = (extension) => {
...
export const normalizeFilename = (value, fallback = DEFAULT_FILE_VIEWER_SOURCE_FILENAME) => {
...
export const resolveFileViewerSourceFilename = ({ filename, file, url, fallback = '', }) => {
    if (filename) {
...
export const normalizeSource = (source) => {
    ...
    const filename = normalizeFilename(source.filename || getBlobName(source.file) || source.url, source.type ? `preview.${normalizeFileExtension(source.type)}` : DEFAULT_FILE_VIEWER_SOURCE_FILENAME);
    const extension = normalizeFileExtension(source.type || getExtension(filename));
    const sourceSize = typeof source.size === 'number'
        ? source.size
        : source.file
            ? source.file.size
            : source.buffer
...
export const wrapFileViewerFileRef = (data, filename = DEFAULT_FILE_VIEWER_SOURCE_FILENAME) => {
...
export const readFileViewerBuffer = async (file) => {
...
export const readFileViewerDataUrl = async (source) => {
...
export const readFileViewerText = async (source, encoding = 'utf-8') => {
    const blob = toFileViewerBlob(source);
