...
const MAX_TIFF_PIXELS = 24000000;
const getBase64Payload = (source) => {
...
const decodeBase64Head = (payload, atob) => {
    try {
...
const hasTiffHeader = (binary) => binary.length >= 4 && ((binary.charCodeAt(0) === 0x49 &&
...
const decodeBase64 = (payload, atob) => {
...
const getTiffDimension = (ifd, tag, fallback) => {
...
const assertSafeTiffDimensions = (width, height) => {
...
export const createSpreadsheetImageSourceResolver = (documentRef) => {
    const view = documentRef.defaultView;
    const decodeAtob = (view === null || view === void 0 ? void 0 : view.atob.bind(view)) || globalThis.atob.bind(globalThis);
    const urlApi = (view === null || view === void 0 ? void 0 : view.URL) || URL;
    const cache = new Map();
    const objectUrls = new Set();
    let disposed = false;
    const decodeTiff = async (source, payload) => {
