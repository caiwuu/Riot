...
const UNSAFE_CONTENT_RE = /<(script|style|noscript|iframe|object|embed|svg)\b[^>]*>[\s\S]*?<\/\1\s*>/gi;
function asciiPreview(bytes, length = 1024) {
    return Array.from(bytes.subarray(0, Math.min(length, bytes.length)), byte => byte === 0 ? '' : String.fromCharCode(byte)).join('');
}
export function isHtmlWordDocumentStream(bytes) {
...
function decodeHtmlWordDocument(bytes) {
...
function createParagraph(source, index) {
    ...
    if (alignment === 'center')
        paraState.alignment = 1;
    if (alignment === 'right')
        paraState.alignment = 2;
    if (alignment === 'justify')
        paraState.alignment = 3;
    const indentPoints = Number.parseFloat(/text-indent\s*:\s*([\d.]+)pt/i.exec(style)?.[1] || '');
    if (Number.isFinite(indentPoints))
        paraState.firstLineIndent = Math.round(indentPoints * 20);
    ...
    const inlines = source.text
        ...
        ...(lineIndex ? [{ type: 'lineBreak' }] : []),
        ...(line ? [{ type: 'text', text: line, style: { ...charState } }] : []),
    ]);
    return {
        type: 'paragraph',
...
export function parseHtmlWordDocumentStream(bytes) {
    if (!isHtmlWordDocumentStream(bytes))
        return null;
