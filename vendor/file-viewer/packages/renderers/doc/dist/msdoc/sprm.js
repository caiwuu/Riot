...
const VARIABLE_OPERAND_CODES = new Set([
...
export function getSprmGroup(sprm) {
    return (sprm >> 13) & 0x7;
}
...
function parseBrc80(bytes, offset = 0) {
    if (offset + 4 > bytes.length)
        return null;
    const brc = u32(bytes, offset) >>> 0;
    if (brc === 0xffffffff)
        return { raw: brc, nil: true };
...
function parseTcgrf(bytes, offset = 0) {
    const value = u16(bytes, offset);
    return {
        raw: value,
        horzMerge: value & 0x3,
...
function parseTc80(bytes, offset = 0) {
...
function parseShd80(bytes, offset = 0) {
    if (offset + 2 > bytes.length)
        return null;
...
function parseShd(bytes, offset = 0) {
...
function parseTableBorders(bytes, legacy) {
    const borderSize = legacy ? 4 : 8;
    if (!bytes.length || bytes.length < 1 + borderSize * 6)
        return null;
...
function parseDefaultShading(bytes, start, legacy) {
...
function parseTDefTableOperand(bytes) {
    if (bytes.length < 3)
        return null;
    ...
    return { cb, numberOfColumns, rgdxaCenter, cells };
}
function parseRangeValueOperand(bytes) {
    if (!bytes.length)
        return null;
    const cb = bytes[0] ?? 0;
    return {
...
function parseRangeWidthOperand(bytes) {
    if (!bytes.length)
        return null;
    const cb = bytes[0] ?? 0;
...
function parseCssaOperand(bytes) {
    if (bytes.length < 7)
        return null;
...
function parseRangeBrcOperand(bytes) {
    if (bytes.length < 8)
        return null;
    return {
        cb: bytes[0] ?? 0,
...
function parseRangeBrcOperand97(bytes) {
    if (bytes.length < 12)
        return null;
    return {
        cb: bytes[0] ?? 0,
...
function parseTableShadeOperand(bytes, odd = false) {
    if (bytes.length < 13)
        return null;
    return {
        cb: bytes[0] ?? 0,
...
function parseTInsertOperand(bytes) {
    if (bytes.length < 4)
        return null;
    return {
        itcFirst: bytes[0] ?? 0,
...
export function getSprmOperandLength(buffer, offset, sprm) {
...
function boolValue(bytes) {
    return Boolean(bytes[0] ?? 0);
}
function setMeta(kind, name, value, raw, bytes) {
    return { kind, name, value, raw, operandBytes: bytes };
}
export function decodeSprm(sprm, operandBytes) {
    ...
    switch (sprm) {
        ...
        case SprmCodes.sprmCHighlight: {
            ...
            const highlight = { index, color: HIGHLIGHT_COLORS[index] };
        ...
        default: {
            const kind = group === 1 ? 'para' : group === 2 ? 'char' : group === 5 ? 'table' : 'unknown';
            return { kind, name: `sprm_${sprm.toString(16)}`, value: bytes, raw: sprm, operandBytes: bytes };
        }
    }
}
export function decodeGrpprl(buffer, startOffset, endOffset) {
    const properties = [];
    ...
    while (offset + 2 <= endOffset) {
        ...
        if (!operandLength || offset + operandLength > endOffset)
            break;
