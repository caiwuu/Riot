...
//!
//! 第四处是 `permissions/autogenerated/`：由 tauri-build 按 `COMMANDS` 生成、
//! 进版本库。它只写不删，所以命令改名或下线后残留的文件得靠这里盯着。
//!
...
/// `permissions/autogenerated/` 下有权限文件的命令。
fn generated() -> BTreeSet<String> {
    let dir = Path::new(env!("CARGO_MANIFEST_DIR")).join("permissions/autogenerated");
    #[allow(clippy::disallowed_methods)]
    let entries =
...
/// 远程分发表（`src/remote/dispatch.rs`）里的命令名：`match` 的每个
/// `"name" =>` 分支。
fn remote_dispatched() -> BTreeSet<String> {
    let src = read("src/remote/dispatch.rs");
    let body = src
        .split_once("match cmd {")
        .expect("dispatch.rs 里找不到 match cmd")
        .1;
    body.lines()
        .filter_map(|l| {
            let t = l.trim();
...
#[test]
fn 远程分发表和注册清单一致() {
    // 网页版走的是同一批命令。少一条的表现是某个按钮在网页上报"未知命令"，
    // 而桌面上一切正常 —— 那种 bug 只有拿手机试才发现得了。
    let missing: Vec<_> = registered()
        .difference(&remote_dispatched())
        .cloned()
        .collect();
    assert!(
        missing.is_empty(),
        "这些命令没进 remote/dispatch.rs 的分发表，网页版调不到：{missing:?}"
    );
    let extra: Vec<_> = remote_dispatched()
        .difference(&registered())
        .cloned()
        .collect();
    assert!(
        extra.is_empty(),
        "remote/dispatch.rs 分发了没注册的命令：{extra:?}"
    );
}
...
#[test]
fn 生成的权限文件没有孤儿() {
    // tauri-build 按 COMMANDS 生成这些文件，但**不删**下线命令留下的那些。
    // 残留文件不会让任何东西报错，只会让人对着一份多出几条能力的清单做
    // 安全判断 —— 比没有清单更糟。删掉即可，下次 build 不会再生成。
    let orphan: Vec<_> = generated().difference(&declared()).cloned().collect();
    assert!(
        orphan.is_empty(),
        "permissions/autogenerated/ 下这些文件对应的命令已经不存在了，删掉它们：{orphan:?}"
    );
}

#[test]
fn 清单不是空的() {
    // 解析逻辑靠文本匹配，重构一旦改动了它依赖的字面量，上面几个测试会
    ...
    assert!(!remote_dispatched().is_empty(), "没解析到 remote/dispatch.rs 的分发表");
    assert!(!generated().is_empty(), "没读到 permissions/autogenerated/");
}
