...
- `redo.json`：Restore 栈（每次回退压一层现场；前进弹一层）。老格式单对象当一层。一层 = 磁盘切片 + **基线表** + 被截掉的消息。`[不变量]` 文件存在 ⇔ 栈非空：`redo_available` 只看存在性（每次切回会话都要问，一层就是整份基线文件全文，不能为此解析），弹空或读不懂就删文件。

每文件三种状态：`text` / `missing` / `skipped`（二进制或超过 1MiB）。范围只覆盖当前会话基线（Edit / Write / 子 agent 记到父会话的那些），**不解析 Bash、不扫整仓**。跳过的文件必须报出来，不能假装回退成功。
