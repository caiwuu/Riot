...
**递归靠深度计数器**。普通子 agent 的工具清单里没有 Task,递归在结构上不存在。分叉是例外:工具清单是请求形状的一部分,少一个 Task 前缀缓存就全失(~100k 的上下文重算一遍),而缓存正是分叉比"重述上下文"划算的全部理由。所以分叉里的 Task **同名、同 schema、同描述**,只是 `depth = 1`:能开同步子 agent,拒绝 `resume: "self"`,后台请求降成同步(通知投递到会话的队列,而分叉不是会话)。
