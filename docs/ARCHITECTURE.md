...
**Delete 是第三个会记基线的文件工具。** 它存在不是因为模型删不了文件(Bash 的 `rm` 一直可以),而是两件 `rm` 给不了的事:弹窗里摆出要删的正文让用户确认,以及永不被 acceptEdits 自动放行(字面路径的 `rm` 现在也被 §8.6 的 Bash 比对记上基线,但那条路弹窗里只有一行命令)。Delete 在删之前把正文原样读下来 `note_baseline`,之后走的就是 Write 新建文件那条路的镜像。
...
**递归靠深度计数器,两层到顶**。主 agent 的 Task 是深度 0;它派的 `general-purpose` 子 agent 和分叉拿到深度 1 的 Task —— **同名、同 schema、同描述**,只是运行时不同:能开同步子 agent,拒绝 `resume: "self"`,后台请求降成同步(通知投递到会话的队列,而子 agent 不是会话)。深度 1 派出去的清单里没有 Task,结构上封死;`explore` 任何时候都没有(只读再拆一层没意义,便宜档账单翻倍)。给子 agent Task 的理由是 Cursor 同款:worker 自己判断"这活可以拆两条线"比主 agent 预先想好拆法更准。分叉必须带 Task 还因为工具清单是请求形状的一部分,少一个前缀缓存就全失(~100k 的上下文重算一遍)。子 agent 派的子 agent 登记 `parent`,`task.history` 连后代一起回,界面把它们缩进挂在父那行下面(树形)。

`[约束]` `TaskTool::prompt` 和 schema 不得随 `depth` 变。变了分叉的第一次请求就不命中缓存,这条约束由分叉的存在理由撑着。

**分叉种子**(`ForkSeed`)在 run_inner 装配完就存在会话上:system、工具清单(Task 已换成 depth 1 那份)、prompt 上下文、延迟池、权限闸、provider、模型、上限、思考策略。不存 `Arc<Registry>` —— 父的注册表里有 depth 0 的 Task,它的 deps 能摸到会话(`Weak`),存注册表回来就是引用环。分叉的历史取**此刻**的活历史,末尾悬空的 tool_use 由 `fork_prelude` 补齐(把它分叉出来的那次调用标"这就是你",兄弟调用标"结果在父那边")。调度器走父的 `build_scheduler`:同一个文件状态缓存(分叉继承了父读过什么,改动追踪也记同一本账)、同一个沙箱、同一个闸。
...
**计划带待办。** `CreatePlan` 的 `todos[]`(对照 Cursor CreatePlanArgs 的 todos,去掉了 id —— Riot 的 TodoWrite 没有 id)是计划拆出的任务清单**初稿**,住在那次调用的参数里,不写进文件、不另存状态:Riot 的待办清单一直是"历史里最后一次 TodoWrite 的输入",计划的待办只是它的起点。三处按同一条规则取(`riot_tools::tools::plan::todos_of` 是形状的唯一定义):内核 `riot_kernel::plan::latest_plan` 在**计划之后还没有任何 TodoWrite** 时把待办随计划附给模型(列在 `<plan_file>` 之后),一旦模型开始用 TodoWrite 跟踪就不再列 —— 它自己最后那次调用才是权威,再摆初稿就是两份矛盾的清单;前端 `planTodos` 同一规则的显示侧,计划面板正文上方画进度区(初稿全 pending,构建后随模型的 TodoWrite 就地翻状态,全部完成自动收起);构建提醒说的是"待办已经定义好了,照着附件里那批、同一措辞调一次 TodoWrite 落成清单,别另起一份"(对照 Cursor 的 "Todos from the plan have already been created"),计划没给待办时才从步骤里拆。
...
**构建键在输入框。** 计划落定、还在规划模式、空闲、输入框没字时,发送键换成「构建 ▾」(Cursor 的 Build);打字就是在提意见,发送键回来。点「构建」:Composer 先把权限模式切回**进规划前那一档**(`execMode`,不是内核建议的兜底值)、**等宿主确认**,再发一条正文只有"开始构建计划"的用户消息,带 `TurnInput::nudge = BuildPlan`;内核把 `prompt::nudge_build_plan` 附在正文之后(以附上的计划文件为准 —— 用户可能改过 —— 落成待办、动手),并在 `MessageMeta.nudge` 上记下是哪个键发的。对话流里这条**不画成用户气泡**,画成一张用户侧的构建卡(`BuildCard`:「构建」/「并行构建」标签 + 构建的那份计划的标题,点开右侧计划面板)—— 对照 Cursor 的 Build 气泡(正文为空、按 `isPlanExecution` 画卡):用户按的是键,不是在说话,那句正文只是给模型和摘录看的占位。计划标题取这条消息之前最近一份计划,同一会话第二次规划再构建时不会指到旧的那份;乐观气泡从一开始就画成卡,不等宿主回声。「并行构建」同路,多切一个多任务开关、提醒换成 `nudge_build_in_parallel`。顺序不能反:宿主在 submit 那一刻打包 TurnConfig,切模式的 IPC 还在路上的话这一轮仍按规划跑。

**当前计划每轮随消息附上**(`prompt::current_plan_context`,对照 Cursor 每轮附的 `Current plan mode progress` + `<file_contents path="cursor-plan://…">`)。"当前计划"由 `riot_kernel::plan` 从**整份记录**(界面归档 + 活历史)里派生:最近一次成功的 CreatePlan,路径取结果首行(结果被轻档压缩清掉时按 `plan_rel_path` 从参数重算),内容每轮从磁盘现读 —— 和前端 `latestPlan` 同一个定义、同一个数据源,面板画的、构建键指的、注给模型的永远是同一份文件;文件被删就是没有计划。规划中与构建后都附,只换一句指引(改这份文件,不用先 Read / 照着做,别改它)。不附的话计划文本只活在模型的 CreatePlan 参数和 Read 结果里,轮次一多或压缩一次就没了,执行到一半偏离计划就是这么来的。

**规划模式的提醒分两版**(`prompt::plan_mode_reminder`,消息侧注入,理由见 §7.6),版本由 `Session::plan_turn` 定:**上一轮结束时就在规划模式**(`prev_turn_mode`,对照 Cursor 的 `prevTurnMode === PLAN`)**且当前计划存在**,讲"用户的话是**改计划**还是**要执行**"—— 规划模式下绝大多数话是改计划("用 Redis"是让你写进计划),只有明确指着计划说"执行"才走 SwitchMode 请用户确认;否则讲"怎么调研、提交前把关键决定定下来、计划里只给一条路线、提交后别问要不要开始(用户有构建键)"。旧判据"历史里出现过 CreatePlan"有两种错法:同一会话第二次进规划会被告知去改上一份(早就构建完的)计划;压缩把那次调用吞掉之后又退回"还没计划",模型重新调研再交一份,面板跳到新文件。文本住在 `riot_tools::tools::plan`(SwitchMode 切进规划模式时要在工具结果里给同一份,riot-tools 不能依赖内核)。

**模式建议由模型发起。** `SwitchMode`(`riot_tools::tools::mode`,对照 Cursor 的同名工具)的 `check_permissions` 返回 `Ask`,理由 `UserChoice`,`suggestions` 带一条 `SetMode`;前端按 `tool_name` 认出它,渲染成对话流里的一张卡(`ModeSwitchCard`:模型给的理由原文 + 「切换」/「不用」)。用户点「切换」,宿主放行时把 SetMode 落到 `mode_live` 上 —— 同一轮内立即生效,模型的下一个工具调用已经按新模式判定;`ModeChanged` 事件让 composer 跟上。拒绝时模型收到"用户不想切,别再问"。system prompt 里有一节 `mode_selection` 讲**什么时候**建议规划(任务大、含糊、有取舍;时机是"动手改之前",看一眼代码估量任务是允许的 —— 要求"读文件之前就决定"的旧措辞让模型没看代码就编理由弹卡),判据和例子留在工具描述里;会话**首条**用户消息末尾再附一句同样的提醒(`prompt::agent_mode_reminder`,给对静态段不敏感的模型兜底),之后的消息不再催 —— 每条都催的结果是中等任务也动不动弹一张「切到规划?」。`[约束]` 理由必须是 `UserChoice`:它让「全部放行」和无人值守也把卡片弹出来,也让 Auto 模式的判危不去替用户答 —— 切模式是用户的决定,不是权限问题。切模式的询问吃一小时超时而不是普通的 60 秒:到点按拒绝处理的话,模型会在没人点头的情况下把一个大改动做下去,而这正是它建议先规划的原因。
...
对话截断（重新生成 / 编辑后重发 / Restore）本来只改历史。文件要跟着回到那条提问发出时的样子，靠旁路检查点，**不进 transcript**。

每个会话一个目录 `sessions/<id>.checkpoints/`：

- `<userMsgId>.json`：用户消息入历史、工具还没跑时拍的磁盘切片
- `head.json`：最近一轮结束后的磁盘（脏检查用）
- `redo.json`：Restore 栈（每次回退压一层现场；前进弹一层）。老格式单对象当一层。一层 = 磁盘切片 + **基线表** + 被截掉的消息。`[不变量]` 文件存在 ⇔ 栈非空：`redo_available` 只看存在性（每次切回会话都要问，不能为此解析），弹空或读不懂就删文件。
...
每文件状态：`blob` / `text`（老格式）/ `missing` / `skipped`（二进制或超过 1MiB，或正文没存进 blob 库）。范围只覆盖当前会话基线（Edit / Write / Delete、Bash 按字面比对出来的、子 agent 记到父会话的那些），**不扫整仓、不监听文件系统**。跳过的文件必须报出来，不能假装回退成功；写回时 blob 找不到了报 `blob_missing`，同样进 `failed`。

`[取舍]` **拦截工具的写，而不是监视磁盘。** 这和 Cursor 的检查点是同一条路：只有 Agent 工具碰过的文件进切片，用户在 IDE 外的手改、数据库 migration 这类副作用一律不覆盖，真正的托底是 git。曾考虑轮次期间 watch 工作区补上 Bash 的增删，否掉了：一次 `npm install` 是几万个事件，得维护一份和 gitignore 几乎重复又永远赶不上的排除表；监控到的"新建"分不清是 Agent 写的、用户手建的还是构建产物；回退时若跟着这些走，会去动不该动的树。

补漏洞走两条路，都不碰监控：

- **把删除做成工具。** `Delete` 记下删前正文作基线，改动栏于是能显示「已删除」，Restore 把它写回来、Redo 再删掉 —— 和 Write 新建的文件走的是同一条 `note_baseline` 路，检查点这边不用为删除加特例。只删单个文本文件（目录 / 二进制指回 Bash）。
- **Bash 按字面比对。** 模型复制一个文件天然会用 `cp`，不盖它的话「新增文件回退不掉」会反复出现。`riot_permissions::bash::effects` 从权限层已有的语法树里读出 `cp` / `mv` / `touch` / `tee` / `rm` / `sed -i` 的路径参数和 `>` / `>>` 的目标（全是**字面量**：变量、命令替换、控制流、后台 `&`、`pushd` / `popd`、带选项或无参数的 `cd` 任一出现整条放弃，glob 只作废那条子命令；字面路径的 `cd <目录>` 则按出现顺序拼进之后的相对路径 —— `cd /proj && rm -f x` 是模型最常用的写法之一）；`riot_tools::tools::bash_effects` 在执行前拍这些候选的前像、执行后再看一眼，**变了才记**。候选可以宁多勿少 —— `cp a d` 里 d 是目录、`sed` 的脚本被当成文件名、`cp a b` 的另一落点 `b/a`（执行后报 ENOTDIR，要按"不存在"处理）—— 前后一致的自然落空。每条命令只多几次 metadata / read。`[约束]` 宁可漏记，不能记错：记错一条基线，回退会按它去改一个不该动的文件；漏记只是回到"终端改动看不见"。前像只认文本（基线是 `Option<String>`）；本来不存在、之后成了二进制的记 `None`（回退删它，是对的），成了目录的不记。脚本、生成器、`npm install`、`git checkout` 仍然看不见，确认框的文案要说清这条边界。

`[约束]` 界面的改动栏在 **Bash 跑完也要重算**，不只是 Edit / Write / Delete。Write 新建、再被 `rm -f` 清掉的文件，内核按「建了又删等于没动过」直接丢掉，但改动栏只在刷新键变化时重新拉一次 —— Bash 不计入的话那条「新增」会一直挂着（真实事故：清理临时文件之后改动栏仍显示两个 `A`）。Cursor 没有这个问题是因为它坐在 VS Code 上，编辑器自带的工作区 watcher 会把磁盘上的删除推给所有视图；我们没有那层，就在每个会动文件的工具跑完时重算一次。
...
Restore C 留下提问 C，丢掉它的回复和之后的 D/E。transcript 追加 `Record::Rewind`。Redo 追加 `Record::Unrewind`：加载时用栈把上次 Rewind 丢掉的尾巴接回（消息仍在 JSONL 里）；活会话从 `redo.json` 弹一层现场。连续回退会叠多层，前进一层后更早的回退还能再前进。Resend / Regenerate / 新提问清掉整栈；`delete_message` 和 `/compact` 也清 —— 历史形状变了，截下的尾巴接不回原位。撤回的提问（模型没开口就停）连它的切片一起删。

`[约束]` Resend / Regenerate 的顺序是**先校验并截断历史，再按检查点写盘，再清 Redo**。反过来的话，目标不合法（归档里的、不是提问的）时会出现"报了错、文件却已经被回退、Redo 栈还被清了"且无路可退。写盘会盖掉用户上一轮之后的手改，所以界面在两条路上都先走 `history.restore_preview`，真会改到文件才弹确认 —— 「重新生成」不能因为按的不是「发送」就静默动手。

`[约束]` 历史被截短时必须 `tasks.cancel_all()`，而且要在拍现场 / 写盘**之前**喊。截掉的轮次后面可能还有子 agent 在写文件，不取消的话它们会继续往已经"没发生过"的工作区里落盘；cancel 只是令牌，尽力而为。

`[约束]` 检查点和基线一样随会话删。内核 `manager.delete` 和宿主 fallback 都要摘目录，漏一边会留下按会话 id 命名的整份源码切片。
...
`[约束]` `dontAsk` 模式和 `can_prompt_user == false`(没有 UI 的异步 agent)时,ask **收敛为 deny**。后台子 agent(§7.6)**不**属于这一类:它和父共用同一个 `HostGate`,闸是会话级的、弹窗照常到界面;只是弹的时候父轮次可能已经结束,所以询问的措辞要能让用户认出"这是后台任务想做的事"。
...
1. 逐次询问的模式下写操作要用户点头,弹窗显示**解析后的绝对路径**——用户能看出这次写的是不是项目外面;
2. §9.4 的安全检查覆盖敏感目标(SSH、凭证、shell 启动脚本、`.git` 内部、本应用配置),且**对「全部放行」免疫**;
3. Bash 命令的静态分析(§9.3)。

`[约束]` **第 1 层不在出厂配置里。** 新会话的出厂档是「全部放行」(`config::default_permission_mode`),它正是"替用户回答例行询问"那一档——于是默认状态下真正拦着的只有第 2、3 层,外加命令隔离(§9.6)。所以那两层的强度不是"多一道保险",是底线:任何削弱它们的改动都直接落在出厂配置上,不再有一个"反正默认会问一下"来兜。

命令隔离本身也**只有 macOS 默认开着**:Windows 那套要先提权装一次(§9.6),没装时开关开着也激活不了,出厂档因此是不隔离。也就是说 Windows 的出厂组合是「全部放行 + 不隔离」,可写范围就是"整块磁盘减去那张敏感清单"。设置页的模式描述、`danger` 标记和二次确认是这个组合下用户唯一的知情来源。
...
`[约束]` **`count_tokens` 只能量"一份完整的、刚发过的历史"**。打底那条 assistant 的 usage 是它那次请求时**整个上下文**的大小,只在"它前面的东西仍是当前历史的前缀"时成立。给它一个子切片(压缩要保留的尾巴)或一份刚拼出来的新历史,打底值就是别的上下文的尺寸,结果大出一个量级、没有报错。那些场合用 `Provider::estimate_tokens_of`(只看内容,不打底)。这条曾经坏过:`split_point` 用 `count_tokens` 量尾巴,尾巴里的 assistant 一打底就是三十万,永远超 20k 预算 —— 线上从没保留过尾巴,每次压缩模型都失忆;测试替身没有 usage,全绿。

`[约束]` 压缩后原样保留的尾巴要 `Message::forget_usage`。它们的 usage 描述的是压缩前的上下文,留着的话新历史下一次被 `count_tokens` 量出来仍是旧尺寸:`after` ≈ `before`、环境档位继续报满、下一轮开工又判超阈值再压一次(总结的总结)。
...
`context_window` 由**用户按模型填**(`ModelConfig::context_window`——输入框的模型菜单里选常用档位,或在「设置 → 服务方 → 模型」里填精确值)。Riot 猜不到窗口:配置里的模型名对不上任何一张公开价目表(中转、自建、微调名都在改名字)。没填的模型退回全局 `compact_threshold_tokens`(默认 300k)。

`[取舍]` 这个兜底值往**高**了取。猜高只是晚压几轮,真顶穿了还有 §10.3 的 413 反应式压缩;猜低是每个会话都被无谓地砍一次历史 —— 压缩吞掉的旧工具结果找不回来,而且每压一次要多付一次模型调用。窗口小的模型该做的是把窗口填上(填了就按窗口算,这个数不参与)。
...
### 10.6 会话摘录:压缩归档 + 跨会话回忆

每个会话有一份**按项目分目录、可 grep 的 Markdown 派生文件**(摘录),同时服务两件事:

- **压缩归档**:压缩后的续接消息(§10.4)指着它,模型要报错原文、具体路径、用户某句原话时 Grep 它、Read 一小段,而不是靠猜。三条压缩路径(主动、手动 `/compact`、413 反应式)都指同一份文件;反应式路径由 `Layered::with_archive` 把路径带进主循环。
- **跨会话回忆**:系统提示词把本项目的摘录目录指给模型,它能回答「上次那个问题最后怎么解决的」(对照 Cursor 的 `<agent_transcripts>`:它把 `~/.cursor/projects/<键>/agent-transcripts/` 写进提示词,剩下交给通用的 Grep/Read)。

`[约束]` **不能让模型直接读 transcript**,三个原因:JSONL 一行一条完整消息、含工具结果全文,Grep 工具按行返回、有 30k 字符上限,命中一次就把返回额度吃光;transcript 是事件日志(编辑/删除/撤回/重新生成是追加的记录,原始消息还在),直接 grep 会把用户明确删掉的内容翻出来;所有项目混在一个目录,靠首行 `meta.root` 区分,容易串台。
...
`[约束]` 摘录必须从**回放后**的历史渲染、整体重写,不能追加。曾经的 `artifacts/<会话>/history.md` 是 append-only 的,用户删掉的消息会留在里面被模型翻出来 —— 那正是摘录要解决的问题,合并之后那条写路径删了。

提示词 `past_sessions` 一节在**项目组**(目录逐用户不同,不能进跨用户共享的静态段;对同一项目的所有会话是常量,项目块缓存不受影响)。当前会话自己的 id 走**消息侧**(首条消息与压缩续接消息里的 system-reminder)—— 放系统提示词会让同项目不同会话的项目块字节不同,白丢一层缓存。引用格式 `[标题](riot://session/<id>)`,前端 `Markdown.tsx` 渲染成芯片、`sessionLink.ts` 切会话,会话已删则就地提示。

开关 `AppConfig::session_recall`(默认开)由宿主经 `digest.configure` RPC 同步给内核(和 MCP 清单同一节拍:启动时、每次保存设置后)。它只管**跨会话**那一半:关掉后提示词不提目录、INDEX.md 收掉、不对账;每个会话自己的摘录照写 —— 压缩归档不是用户能关的。重新打开触发一次对账把 INDEX 补回来。不做 Cursor 那样的 30 天自动清理:摘录和会话一一对应,会话在侧栏可删,删了摘录就跟着没了。
...
`[约束]` 第 3 层的粒度是**轮**,不是块。`content-visibility` 跳过的是元素的*内容*,盒子本身仍要每帧参与列布局、做视口邻近判定、维护 `contain-intrinsic-size: auto` 的"上次记住的尺寸"——这笔记账按 `content-visibility` 元素**个数**算。每块一份时它随会话长度线性增长:侧栏 300ms 开合动画里 `.main` 每帧变宽、整棵子树重排,实测每帧 style+layout 从 150 块的 0.07ms 涨到 3000 块的 1.41ms(一帧预算 8.3ms);按轮包一层后曲线基本压平(0.2~0.35ms,与块数无关)。同一组数字在完全不用这条时是 0.8ms / 26ms——所以这个优化是承重的,只是粒度要选对。(Codex 桌面端同款:挂在 `[data-virtualized-turn-content]` 上,一轮一份,估高 240px。)

配套的两个逃生舱都在轮那一层:正在生成的最后一轮挂 `data-live` 退出优化(内容每帧在长,估高一直失效);编辑态用 `.turn:has(.msg.editing)` 关掉(膨胀的元素可能嵌在轮内任意深度)。锚点补偿因此改成两级二分(先定位轮、再进轮内定位块),精度和每块一份时一致。
