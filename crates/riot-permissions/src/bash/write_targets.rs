...
//!
//! 这些目标全在沙箱边界**之内**。所以要在放宽之前挡住它们。
//!
//! # 和 OS 排除面的分工
//!
//! `[取舍]` cargo 敏感面（config / bin / 凭证）**同时**由 OS 挡着 ——
//! seatbelt 的 deny 子句、Windows 的保护洞（`riot_runtime::sandbox` 的
...
//!
//! **工作区内**的目标（`.git/hooks/`、`.riot/hooks.json`）则只有这一层，
//! 刻意不进 OS 排除面：装 husky、写 hooks 是高频合法操作，OS 级 deny
//! 会造出「用户点了允许、命令照样失败」——沙箱按静态策略每轮激活，不知道
//! 用户这一次批准了什么。它们的间接写入是接受的残余风险，见
