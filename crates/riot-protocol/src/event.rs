...
pub enum AgentEvent {
    ...
    /// 会话的权限模式变了，而且**不是**用户在界面上的模式菜单里切的。
    ///
    /// 目前唯一来源：模型调用 SwitchMode、用户在对话流里的卡片上点了
    /// 「切换」，内核当场把模式落到会话上。界面必须跟着更新 —— 否则
    /// composer 还显示「Agent」，而内核已经按规划模式拦写操作了（或者
    /// 反过来：还显示「规划」，宿主已经在放行编辑）。显示得比实际更严
    /// 是最坏的一种错（和 SessionInfo.mode 的注释同一条教训）。
    ModeChanged {
