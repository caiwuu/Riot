...
//!
//! 而 Edit 自己从磁盘载入全文、Write 要求先 Read，所以每个被改的文件
//! 在动手之前都经过文件状态缓存 —— 那份内容就是天然的基线，精确到
//! "本会话经工具写下的改动"。项目不在 git 里也照样能用。
//!
//! `[已知局限]` Bash 只认**按字面写出路径**的那几条命令（`cp` / `mv` /
//! `touch` / `tee` / `rm` / `sed -i` / 重定向，见 `riot_tools::tools::
//! bash_effects`）：执行前后比对，变了才记。脚本、生成器、`npm install`、
//! `git checkout` 改的文件这里看不见 —— 抓全要么解析 shell 语义，要么
//! 监听磁盘，两条都不走（理由见 ARCHITECTURE.md §8.6）。
//!
